  ...
  <div class="max-w-[1000px] mx-auto px-4">
    ...
    <n-card style=" background-color: transparent;" :bordered="false">
      <n-data-table :columns="columns" :data="data" :pagination="{ pageSize: 10 }" :row-props="rowProps"
        :row-key="(row: EncryptLv) => row.id" :scroll-x="800" />
      ...
      <n-modal v-model:show="showModal" preset="card" style="width: 500px;" :header-style="{ paddingBottom: '8px' }">
        ...
        <n-form v-if="selectedRow">
          ...
          <n-form-item :show-feedback="false">
            <n-input-group>
              <n-tag type="info"
                style="height: 90px;width: 150px; display: flex; align-items: center; justify-content: center;">
                Description
              </n-tag>
              <n-input v-model:value="selectedRow.descr" placeholder="" type="textarea" :autosize="{
                minRows: 3,
                maxRows: 5,
              }" />
            </n-input-group>
